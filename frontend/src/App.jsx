...
function App() {
  ...
  // Note: the browser and engine both read from the engine's MJPEG endpoints, so if the
  // browser and the engine both try to open the phone's URL directly, one of
  // them gets an error instead of video.
  ...
  const [camera1Url, setCamera1Url] = useState(WEBCAM_URL);
  ...
  const [cameraSyncStatus, setCameraSyncStatus] = useState({
    type: "INFO",
    message: "Webcam ready. Click Connect Camera to start.",
  });
  ...
  const connectCamera1 = () => {
    ...
    setCameraSyncStatus({
      type: "INFO",
      message: "Connecting to camera... (webcam default)",
    });
  ...
  const handleCamera1Load = () => {
    ...
    setCameraSyncStatus({
      type: "SUCCESS",
      message: "Camera #1 is LIVE (webcam).",
    });
  };
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 flex-grow w-full netra-main-content netra-page-enter">
        ...
        {activePage === "livefeed" && (
          <div className="space-y-6">
            ...
            {/* CAMERA GRID — Camera #1 + AI Feed side by side */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              {/* AI PROCESSED FEED — now inside the grid, side by side */}
